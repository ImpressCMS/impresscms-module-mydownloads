<?php
// $Id: xoopstree.php 506 2006-05-26 23:10:37Z skalpa $
//  ------------------------------------------------------------------------ //
...
class mydlXoopsTree
{
	...
	function mydlXoopsTree($table_name, $id_name, $pid_name)
	{
	...
	function mydl_makeMySelBox($title, $order='', $preset_id=0, $none=0, $sel_name="", $onchange="")
	{
		$buffer = '';
		if ( $sel_name == "" ) {
			$sel_name = $this->id;
		...
		$myts =& MyTextSanitizer::getInstance();
		$buffer .= "<select name='".$sel_name."'";
		if ( $onchange != "" ) {
			$buffer .= " onchange='".$onchange."'";
		}
		$buffer .= ">\n";
		...
		while ( list($catid, $name) = $this->db->fetchRow($result) ) {
			...
			$buffer .= "<option value='$catid'$sel>$name</option>\n";
			$sel = "";
			...
			foreach ( $arr as $option ) {
				...
				$buffer .= "<option value='".$option[$this->id]."'$sel>$catpath</option>\n";
				$sel = "";
			}
		}
		$buffer .="</select>\n";
	...
	/**
	 * Create an array of the topics
	 *
	 * @param string $title	name of the field to use as a title
	 * @param string $order	Field to order datas on
	 * @return array	Key = Topicd Id, Value = array('title', 'pid')
	 */
	function mydl_makeTreeAsArray($title, $order = '')
	{
		$ret = array();
		$myts =& MyTextSanitizer::getInstance();
		$sql = "SELECT ".$this->id.", pid, ".$title.' FROM '.$this->table.' WHERE '.$this->pid.' = 0';
		if ( $order != '') {
			$sql .= " ORDER BY $order";
		}
		$result = $this->db->query($sql);
		while ( list($catid, $pid, $name) = $this->db->fetchRow($result) ) {
			$ret[$catid] = $name;
			$arr = $this->getChildTreeArray($catid, $order);
			foreach ( $arr as $option ) {
				$option['prefix'] = str_replace('.', '--', $option['prefix']);
