...
include_once XOOPS_ROOT_PATH.'/modules/mydownloads/class/xoopstree.php';
...
function listNewDownloads()
{
	...
	if ($numrows>0) {
		while(list($lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $uid) = $xoopsDB->fetchRow($result)) {
			...
			// **************************************************************************************************
			include_once XOOPS_ROOT_PATH."/class/xoopsformloader.php";
			$sform = new XoopsThemeForm(_MD_MODDL, "filefom", XOOPS_URL.'/modules/'.$xoopsModule->getVar('dirname').'/admin/index.php', 'post');
			//$sform->setExtra('enctype="multipart/form-data"');
			$sform->addElement(new XoopsFormLabel(_MD_SUBMITTER,"<a href=\"".XOOPS_URL."/userinfo.php?uid=".$uid."\">$submitter</a>"),false);
...
{
	global $xoopsDB, $myts, $eh, $xoopsModule, $xoopsModuleConfig;
	...
	$form->addElement(new XoopsFormFile(_MD_UPLOAD_FILE, 'attachedfile', $xoopsModuleConfig['maxuploadsize']), false);
	...
	if($numrows>0) {
		...
		$sform = new XoopsThemeForm(_MD_ADDNEWFILE, 'filefom', XOOPS_URL.'/modules/'.$xoopsModule->getVar('dirname').'/admin/index.php', 'post');
		$sform->setExtra('enctype="multipart/form-data"');
	...
	if($totalmodrequests>0){
		...
		while(list($requestid, $lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $description, $modifysubmitter)=$xoopsDB->fetchRow($result)) {
			...
			if ( $xoopsModuleConfig['useshots'] && !empty($origlogourl) ){
				echo "<img src=\"".$xoopsModuleConfig['shotsuploadfolderurl'].$origlogourl."\" width=\"".$xoopsModuleConfig['shotwidth']."\">";
			}else{
				echo "&nbsp;";
			}
			echo "</td></tr>
			...
			if ( $xoopsModuleConfig['useshots'] && !empty($logourl) ){
				echo "<img src=\"".$xoopsModuleConfig['shotsuploadfolderurl'].$logourl."\" width=\"".$xoopsModuleConfig['shotwidth']."\">";
			} else {
				echo "&nbsp;";
			}
			echo "</td></tr>
...
function modCatS()
{
	...
	if(isset($_POST['xoops_upload_file'])) {
		...
		if(xoops_trim($fldname!='')) {
			...
			$tbl_tmp = @split('|',$xoopsModuleConfig['mimetype']);
			$permittedtypes = @array_walk($tbl_tmp,'trim');
			$uploader = new XoopsMediaUploader( $xoopsModuleConfig['shotsuploadfolderpath'], $tbl_tmp, $xoopsModuleConfig['maxuploadsize']);
			$uploader->setTargetFileName($destname);
			if ($uploader->fetchMedia($_POST['xoops_upload_file'][0])) {
				if ($uploader->upload()) {
					$imgurl = $xoopsModuleConfig['shotsuploadfolderurl'].'/'.$destname;
				} else {
					echo _MD_UPLOAD_ERROR. ' ' . $uploader->getErrors();
				}
			} else {
...
function addCat()
{
	...
	if(isset($_POST['xoops_upload_file'])) {
		...
		$fldname = (get_magic_quotes_gpc()) ? stripslashes($fldname['name']) : $fldname['name'];
		if(xoops_trim($fldname!='')) {
			$destname = mydownloads_createUploadName($xoopsModuleConfig['shotsuploadfolderpath'],$fldname);
			$tbl_tmp = @split('|',$xoopsModuleConfig['mimetype']);
			$permittedtypes = @array_walk($tbl_tmp,'trim');
			$uploader = new XoopsMediaUploader( $xoopsModuleConfig['shotsuploadfolderpath'], $tbl_tmp, $xoopsModuleConfig['maxuploadsize']);
			$uploader->setTargetFileName($destname);
...
{
	global $xoopsDB, $xoopsUser, $xoopsModule, $myts, $eh, $xoopsModuleConfig;
...
default:
	...
	if($result && $totalCount > 0) {
		if (file_exists( XOOPS_ROOT_PATH.'/language/'.$xoopsConfig['language'].'/admin.php')) {
			...
			require_once  XOOPS_ROOT_PATH.'/language/english/admin.php';
		}
