<?php
// $Id: notification.inc.php,v 1.5 2004/12/26 19:11:56 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
function mydownloads_notify_iteminfo($category, $item_id)
{
	...
	if (empty($icmsModule) || $icmsModule->getVar('dirname') != 'mydownloads') {	
	...
	if ($category == 'category') {
		...
		$item['url'] = ICMS_URL . '/modules/' . $module->getVar('dirname') . '/viewcat.php?cid=' . $item_id;
		return $item;
	}
	...
	if ($category=='file') {
		...
		$result_array = $xoopsDB->fetchArray($result);
		$item['name'] = $result_array['title'];
		$item['url'] = ICMS_URL . '/modules/' . $module->getVar('dirname') . '/singlefile.php?cid=' . $result_array['cid'] . '&amp;lid=' . $item_id;
		return $item;
	}
}
?>
