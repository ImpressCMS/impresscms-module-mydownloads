...
if(is_array($categories) && count($categories) > 0) {
	if(!in_array($cid, $categories)) {
		redirect_header(ICMS_URL.'/modules/mydownloads/index.php', 3, _NOPERM);
		exit();
	}
	...
	foreach($arr as $ele){
		if(in_array($ele['cid'], $categories)) {
			...
			foreach($sub_arr as $sub_ele){
				...
				$infercategories .= "<a href=\"".ICMS_URL."/modules/mydownloads/viewcat.php?cid=".$sub_ele['cid']."\">".$chtitle."</a>";
				$space++;
				$chcount++;
			}
...
if (!empty($xoopsUser) && $xoopsUser->isAdmin($icmsModule->mid())) {
	$isadmin = true;
} else {
	$isadmin = false;
}
...
//if 2 or more items in result, show the sort menu
	...
	// Hack Herv� (http://www.herve-thouzard.com)
	$cpt = 0;
	while(list($lid, $dtitle, $url, $homepage, $version, $size, $platform, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description)=$xoopsDB->fetchRow($result)) {
		...
		//$summary .= "<a href='".ICMS_URL.'/modules/mydownloads/singlefile.php?cid='.$cid.'&lid='.$lid."'>".$dtitle.'</a><br />';
...
if($icmsModuleConfig['autosummary']) {
