...
if (empty($xoopsUser) && !$icmsModuleConfig['anonpost']) {
...
*/
	...
	if ( $icmsModuleConfig['autoapprove'] == 1 ) {
		$status = $icmsModuleConfig['autoapprove'];
	} else {
		$status = 0;
	}
	...
	if ( $icmsModuleConfig['autoapprove'] == 1 ) {
		...
		$tags['WAITINGFILES_URL'] = ICMS_URL . '/modules/' . $icmsModule->getVar('dirname') . '/admin/index.php?op=listNewDownloads';
...
*/
	...
	$xoopsTpl->assign('notify_show', !empty($xoopsUser) && !$icmsModuleConfig['autoapprove'] ? 1 : 0);
	...
	include_once ICMS_ROOT_PATH.'/modules/mydownloads/include/functions.php';
	...
	mydownloads_create_meta_description($icmsModule->name().' - '._MD_SUBMITCATHEAD);
	// End Hack
