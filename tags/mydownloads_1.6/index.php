...
$sortorder = isset($xoopsModuleConfig['toporder']) ? $xoopsModuleConfig['toporder'] : 1;
...
$result = $xoopsDB->query($sql, $xoopsModuleConfig['newdownloads'], $start);
while(list($lid, $cid, $dtitle, $url, $homepage, $version, $size, $platform, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description)=$xoopsDB->fetchRow($result)) {
	...
	$pop = popgraphic($hits);
	if ($isadmin) {
		$adminlink = '<a href="'.XOOPS_URL.'/modules/mydownloads/admin/index.php?lid='.$lid.'&fct=mydownloads&op=modDownload" title="'._MD_EDITTHISDL.'"><img src="'.XOOPS_URL.'/modules/mydownloads/images/editicon.gif" border="0" alt="'._MD_EDITTHISDL.'" /></a>';
	} else {
		$adminlink = '';
	}
	if ($votes == 1) {
...
mydownloads_create_meta_description($xoopsModule->name());
// End Hack
