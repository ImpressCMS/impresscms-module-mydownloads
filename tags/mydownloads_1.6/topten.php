<?php
// $Id: topten.php,v 1.10 2004/12/26 19:11:56 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
$arr=array();
$sql = "SELECT cid, title FROM ".$xoopsDB->prefix('mydownloads_cat')." WHERE pid=0";
...
if($sortDB == "rating") {
	mydownloads_create_page_title(_MD_RATINGHTOL);
	mydownloads_create_meta_description($xoopsModule->name().' - '._MD_RATINGHTOL);
} else {
	mydownloads_create_page_title(_MD_POPULAR);
	mydownloads_create_meta_description($xoopsModule->name().' - '._MD_POPULAR);
}
// End Hack
