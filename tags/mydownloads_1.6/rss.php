...
$items_count = $xoopsModuleConfig['newdownloads']; // Number of items to display, by default, the module's preference used to select the count of items to display on the index page
...
if (!$tpl->is_cached('db:mydownloads_rss.html', $cacheId)) {
	...
	if(count($categories) > 0) {
		...
		while($myrow = $xoopsDB->fetchArray($result)) {
			...
			$link = XOOPS_URL.'/modules/mydownloads/singlefile?cid='.$myrow['cid'].'&amp;lid='.$myrow['lid'];
   			$guid = $link;
